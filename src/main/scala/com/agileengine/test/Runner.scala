...
object Runner {
  ...
  /** Iterate through all elements in document
    ...
    * @return
    */
  def findMostAlike(target: Element,
                    in: Document,
                    threshold: Int = 0): Option[Element] = {

    val scorer = Scorer()
    val targets = in.body.select("*")

    // todo: elements list might be empty
    val elements = targets.iterator().asScala.toSet[Element]

    val elementsWithScores = elements.map(el => el -> scorer.score(target, el))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = new scopt.OptionParser[Config]("Find similar elements") {
      arg[File]("<input_origin_file_path>")
        .required()
        .valueName("<input_origin_file_path>")
      ...
      arg[File]("<input_other_sample_file_path>")
        .required()
        .valueName("<input_other_sample_file_path>")
    ...
    parser.parse(args, Config()) match {
      case Some(config) =>
        val result = for {
          ...
          element <- Try(Jsoup.parse(config.in, Charset))
            .map(_.getElementById(config.id))
